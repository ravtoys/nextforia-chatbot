...
function resetTestConversation(){testConversation.forEach(function(message){if(message.preview_url&&message.preview_url.indexOf("blob:")===0)URL.revokeObjectURL(message.preview_url);});testConversation=[{role:"assistant",kind:"text",text:PANEL_TEST_GREETING,context_text:PANEL_TEST_GREETING,time:testChatTime()}];testConversationBusy=false;var input=document.getElementById("testChatInput");if(input){input.value="";input.style.height="";}["testImageFile","testAudioFile"].forEach(function(id){var file=document.getElementById(id);if(file)file.value="";});setTestConversationBusy(false);}
