...
.channelConnectIcon.samsung{background:linear-gradient(145deg,#132A52,#2563C4)}
.samsungCalendarDialog{position:fixed;z-index:220;inset:0;display:grid;place-items:center;padding:20px;background:rgba(5,20,45,.58)}
.samsungCalendarDialogCard{width:min(520px,100%);border-radius:22px;background:#fff;padding:26px;box-shadow:0 28px 80px rgba(5,20,45,.35)}
.samsungCalendarDialogCard h3{margin:0;color:var(--navy-900);font-size:23px;font-weight:950}
.samsungCalendarDialogCard p{margin:9px 0;color:var(--slate-600);font-size:14px;line-height:1.55}
.samsungCalendarDialogOptions{display:grid;gap:10px;margin:18px 0}
.samsungCalendarDialogOptions button{width:100%;min-height:58px;text-align:left}
.samsungCalendarDialogActions{display:flex;justify-content:flex-end;margin-top:16px}
...
function renderAppointmentCalendarGroup(calendars,canManage){calendars=(calendars||[]).filter(Boolean);if(!calendars.length)return"";var active=calendars.find(function(calendar){return calendar.active&&calendar.status==="connected";})||calendars.find(function(calendar){return calendar.active;}),connected=!!(active&&active.status==="connected"),underlying=active&&active.underlying_provider||active&&active.provider||"",underlyingName=underlying==="microsoft"?"Microsoft Outlook":"Google Calendar",account=active&&(active.calendar_summary||active.account_label||active.account_email)||"",actions='<span class="channelState '+attr(connected?"connected":"not_connected")+'">'+esc(connected?"Listo":"No conectado")+'</span>';if(canManage){actions+=calendars.map(function(calendar){var provider=calendar.provider==="samsung"?"samsung":calendar.provider==="microsoft"?"microsoft":"google",providerName=calendar.name||(provider==="samsung"?"Samsung Calendar":provider==="microsoft"?"Microsoft Outlook":"Google Calendar"),providerConnected=calendar.active&&calendar.status==="connected";if(calendar.authorization_available===false)return '<span class="channelAccount">'+esc(providerName)+" · Próximamente"+'</span>';return '<button class="'+(providerConnected?"ghostBtn":"primaryBtn")+'" type="button" data-provider="'+attr(provider)+'" onclick="connectAppointmentCalendar(this.dataset.provider)">'+esc((providerConnected?"✓ ":"")+providerName)+'</button>';}).join("");if(connected)actions+='<button class="ghostBtn" type="button" onclick="disconnectAppointmentCalendar()">Desconectar</button>';}var accountText=connected?"Conectado: "+(active&&active.surface==="samsung"?"Samsung Calendar · "+underlyingName+(account?" · "+account:""):account||(active&&active.name)||"calendario del negocio"):"Elige Google, Microsoft o Samsung. Para Samsung conectaremos la cuenta de Google u Outlook que ya sincroniza tu teléfono.";return '<article class="channelConnectCard'+(connected?' recommended':'')+'"><span class="channelConnectIcon calendar samsung">SC</span><div class="channelConnectCopy"><h4>¿Qué calendario usa tu negocio?</h4><p>Nextfor revisará disponibilidad y creará Citas NextforIA. Samsung Calendar se sincroniza de forma segura mediante Google u Outlook.</p><div class="channelAccount">'+esc(accountText)+'</div></div><div class="channelConnectActions">'+actions+'</div></article>';}
...
function samsungCalendarOptions(){var rows=(state.channelConnections&&state.channelConnections.appointment_calendar_providers)||[],samsung=rows.find(function(row){return row&&row.provider==="samsung";});return samsung&&Array.isArray(samsung.connection_options)?samsung.connection_options:[];}
function closeSamsungCalendarDialog(){var dialog=document.getElementById("samsungCalendarDialog");if(dialog)dialog.remove();}
function openSamsungCalendarDialog(){if(DEMO_MODE){setChannelConnectionMessage("Demo: aquí elegirías Google u Outlook, la cuenta que ya sincroniza Samsung Calendar.","success");return;}closeSamsungCalendarDialog();var options=samsungCalendarOptions(),google=options.find(function(row){return row.provider==="google";})||{authorization_available:true},microsoft=options.find(function(row){return row.provider==="microsoft";})||{authorization_available:true},dialog=document.createElement("div");dialog.id="samsungCalendarDialog";dialog.className="samsungCalendarDialog";dialog.setAttribute("role","dialog");dialog.setAttribute("aria-modal","true");dialog.setAttribute("aria-labelledby","samsungCalendarDialogTitle");dialog.innerHTML='<section class="samsungCalendarDialogCard"><h3 id="samsungCalendarDialogTitle">Conecta Samsung Calendar</h3><p>Samsung Calendar muestra y sincroniza los calendarios de Google u Outlook. Elige la misma cuenta que ya tienes activada en Samsung Calendar; Nextfor creará, reprogramará y cancelará las citas allí.</p><div class="samsungCalendarDialogOptions"><button class="primaryBtn" type="button" onclick="chooseSamsungCalendarProvider(\'google\')" '+(google.authorization_available===false?'disabled':'')+'>Continuar con Google Calendar</button><button class="ghostBtn" type="button" onclick="chooseSamsungCalendarProvider(\'microsoft\')" '+(microsoft.authorization_available===false?'disabled':'')+'>Continuar con Microsoft Outlook</button></div><p class="channelAccount">No pedimos una contraseña de Samsung ni creamos un calendario duplicado. La sincronización de tu teléfono permanece bajo tu cuenta de Google u Outlook.</p><div class="samsungCalendarDialogActions"><button class="ghostBtn" type="button" onclick="closeSamsungCalendarDialog()">Cancelar</button></div></section>';dialog.addEventListener("click",function(event){if(event.target===dialog)closeSamsungCalendarDialog();});document.body.appendChild(dialog);var first=dialog.querySelector("button:not([disabled])");if(first)first.focus();}
function startAppointmentCalendarOAuth(provider,surface){provider=provider==="microsoft"?"microsoft":"google";surface=surface==="samsung"?"samsung":"direct";var name=provider==="microsoft"?"Microsoft Outlook":"Google Calendar",display=surface==="samsung"?"Samsung Calendar con "+name:name;if(DEMO_MODE){setChannelConnectionMessage("Demo: aquí abriríamos "+display+" para conectar el calendario de tu negocio.","success");return;}var externalTab=prepareExternalIntegrationTab(display);if(!externalTab){setChannelConnectionMessage("Tu navegador bloqueó la nueva pestaña. Permite ventanas emergentes para Nextfor y vuelve a intentar.","error");return;}setChannelConnectionMessage(display+" se abrirá en una pestaña nueva…");api("/admin/panel/appointment-calendar/"+provider+"/connect",{method:"POST",body:JSON.stringify({return_to:"channels",surface:surface})}).then(function(body){if(!body.authorization_url)throw new Error("authorization_unavailable");if(!navigateExternalIntegrationTab(externalTab,body.authorization_url))throw new Error("popup_navigation_failed");setChannelConnectionMessage(display+" se abrió en una pestaña nueva. Termina allí y luego vuelve a este panel.","success");}).catch(function(error){closeExternalIntegrationTab(externalTab);setChannelConnectionMessage(error.body&&error.body.message||"No pudimos abrir el calendario. Intenta de nuevo o habla con NextforIA.","error");loadChannelConnections(true);});}
function chooseSamsungCalendarProvider(provider){closeSamsungCalendarDialog();startAppointmentCalendarOAuth(provider,"samsung");}
function connectAppointmentCalendar(provider){if(provider==="samsung"){openSamsungCalendarDialog();return;}startAppointmentCalendarOAuth(provider,"direct");}
